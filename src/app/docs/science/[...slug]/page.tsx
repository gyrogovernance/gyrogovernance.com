    ...
    <>
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="mb-4 sm:mb-6">
          ...
          <Link href="/docs/science" className="text-classic-blue hover:text-classic-purple transition-colors">
            Science
          </Link>
          {slug && slug.length > 1 && (
            <>
...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!slug || !Array.isArray(slug) || slug.length === 0) {
    return {
      ...
      description: 'Science documentation',
    };
  }
  const slugPath = slug.join('/');
  ...
  try {
    ...
    return {
      title: data.title || `${slugPath.replace(/_/g, ' ')} | Science`,
      description: data.description || 'Science documentation',
    };
  } catch {
    return {
      title: `${slugPath.replace(/_/g, ' ')} | Science`,
    };
  }
}